...
[X,Y]=meshgrid([-1*pi:(2*pi/500):1*pi],[-9:0.01:9]);
[x_d,y_d] = size(X);
...
for k = 1:length(xi_temp)
    xi_vali = xi_temp(:,k);
    %xi_vali(2) = xi_vali(2) - 1;
    Z_sos = dV_sos([sin(vecX);cos(vecX);vecY],xi_vali);
    ...
    Z_ccp = dV_ccp([sin(vecX);cos(vecX);vecY],xi_vali);
    ...
    Z_drccp = dV_drccp([sin(vecX);cos(vecX);vecY],xi_vali);
